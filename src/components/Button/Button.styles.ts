 ...
 */
export const StyledButton = styled.button<Omit<ButtonProps, "label">>` /* ✅ Removed label from styled props */
  ...
  background-color: ${({ backgroundColor, primary, disabled }) =>
    disabled ? "#ccc" : backgroundColor ? backgroundColor : primary ? "#007bff" : "#555"};
  ...
  &:hover {
    background-color: ${({ backgroundColor, primary, disabled }) =>
      disabled ? "#ccc" : backgroundColor ? backgroundColor : primary ? "#0056b3" : "#333"};
    transform: ${({ disabled }) => (disabled ? "none" : "scale(1.05)")}; /* ✅ Adds slight hover effect */
  }

  &:active {
    transform: ${({ disabled }) => (disabled ? "none" : "scale(0.95)")}; /* ✅ Click effect */
  }
`;
